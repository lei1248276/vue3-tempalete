...
const options: EChartsOption = {
  tooltip: {
    ...
    formatter: (p: any) => {
      ...
      p.forEach((val: any) => {
        str += `
          <div class="flex items-center gap-x-2 mt-[2px]">
            <span class="w-2 h-2 ml-1 rounded-full" style="background-color: ${val.color};"></span>
            <span>${val.seriesName}</span>
            <span>${val.data >= 10000 ? (val.data / 10000).toFixed(2) + 'w' : val.data}</span>
          </div>
          `
      })
      const dom = `
        <div class="w-[206px] h-[103px] py-1 px-5 bg-[url(@/views/dataScreen/images/contrast-bg.png)] bg-[length:100%_100%] bg-no-repeat text-[10px] text-[#03b8e2]">
          <span>${p[0].dataIndex + 1}月</span>
          ${str}
        </div>
      `
      return dom
  ...
  series: data.data.map((val, index: number) => {
    return {
      name: val.label,
      type: 'line',
